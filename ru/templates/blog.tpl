    ...
    <div class="container">
        ...
        <div class="row sectionIntro">
            ...
            <div class="col-md-12 col-md-offset-0 text-center">
                <h1><span data-scroll-reveal="enter left move 10px over 1s after 0.2s">{if $category != ""}{$category.name}{else}Статьи и новости компании{/if}</span></h1>
        ...
        <div class="row content">
            <div class="col-md-8 blog">
                {foreach from=$articles item=article key=key}
                    <!-- Blog Post-->
                    <article>
                        <img class="img-responsive" src="/img/blog/{$article.image}" alt="" />
                        <div class="post-content">
                            <h2><a href="/article.php?id={$article.id}">{$article.name}</a></h2>
                            <div class="thedate">{$article.createAt} {$article.category_name}</div>
                            <hr/>
                            <p>{$article.short_descr}</p>

                            <a class="button" href="/article.php?id={$article.id}">Продолжить чтение →</a>
                        </div>
                    </article>
                    <!-- End of Blog Post-->
                {/foreach}
                ...
                <nav>
                    <ul class="pagination">
                        {if $smarty.get.page|default:1 > 1}
                            <li>
                                <a href="/blog.php{if $category != ""}?id={$category.id}{/if}{if $smarty.get.id}&page={math equation="x - 1" x=$smarty.get.page}{else}?page={math equation="x - 1" x=$smarty.get.page}{/if}" aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                </a>
                            </li>
                        {/if}
                        {for $foo=1 to $pages}
                            <li><a href="/blog.php{if $category != ""}?id={$category.id}{/if}{if $smarty.get.id}&page={$foo}{else}?page={$foo}{/if}">{$foo}</a></li>
                        {/for}

                        {if  $smarty.get.page|default:1 < $pages}
                            <li>
                                <a href="/blog.php{if $category != ""}?id={$category.id}{/if}{if $smarty.get.id}&page={math equation="x + 1" x=$smarty.get.page|default:1}{else}?page={math equation="x + 1" x=$smarty.get.page|default:1}{/if}" aria-label="Next">
                    ...
                    <div class="widget">
                        ...
                        <ul>
                            {foreach from=$categories item=cat key=key}
                                <li {if $category.id == $cat.id} class="active"{/if}><a href="/blog.php?id={$cat.id}">{$cat.name}</a> <span class="badge">{$cat.count}</span></li>
                            {/foreach}
                        </ul>
                    </div>
